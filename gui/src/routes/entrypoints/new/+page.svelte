...
<section class="max-w-3xl space-y-5">
	<Button href="/entrypoints" variant="ghost"><ArrowLeftIcon />{copy.back}</Button>
	<div>
	...
	<Card.Root class="rounded-lg">
		<Card.Content class="space-y-5 pt-6">
			...
			<div class="grid gap-3 md:grid-cols-2">
				<label class="grid gap-1.5 text-xs text-muted-foreground">{copy.name}<Input bind:value={name} placeholder="home_wg" /></label>
				<label class="grid gap-1.5 text-xs text-muted-foreground">{copy.iface}<Input bind:value={iface} placeholder="wg0" /></label>
				<label class="grid gap-1.5 text-xs text-muted-foreground">{copy.subnet}<Input bind:value={subnet} placeholder="10.66.66.0/24" /></label>
				<label class="grid gap-1.5 text-xs text-muted-foreground">{copy.port}<Input bind:value={port} type="number" placeholder="51820" /></label>
			</div>
			{#if message}<p class="text-sm text-red-300">{message}</p>{/if}
			<Button disabled={saving} onclick={create}><PlusIcon />{copy.create}</Button>
		</Card.Content>
	</Card.Root>
</section>
